...
if(isset($_GET['id']))
{
          $id =$_GET['id'];
          // echo $id;
          $sql_update = "SELECT * FROM employees WHERE id= $id";
          $arr = mysqli_query($conn,$sql_update);
          $row = mysqli_fetch_assoc($arr);
//         echo  date("d/m/Y",strtotime($row['birthday']));
//           echo"<pre>";
//           print_r($row);
//           echo"</pre>";
// echo $show=$row['birthday'];
// echo strtotime($row['birthday']);
// echo $time = time();
// $a = number_format($row['salary'],'0',',','.');
// echo $a."VND";
...
// $gioitinh=$_POST["gioitinh"];
	...
	
	{
                    ...
                    // $id =$_GET['id'];
                    // echo $id;
                    $sql_update = "UPDATE employees SET name ='$name',description='$description',salary=$salary,gender='$gioitinh',birthday='$ngaysinh' WHERE id= $id";
                    $arr = mysqli_query($conn,$sql_update);
                    $row = mysqli_fetch_assoc($arr);
...
<form action="" method="POST" style="width:600px" enctype="multipart/form-data" class="border border-primary boder-2 rounded m-auto mt-5 p-2">
	<h1 class="mb-3 d-block text-center">Update Record</h1>
	<div class="mb-3">
		<label for="name" class="form-label">Name</label>
		<input  type="text" value="<?php echo $row['name'];?>" class="form-control" name="name" id="name" placeholder="tên phải từ 6 đến 20 ký tự">
		<?php if (isset($loiten)) {?>
		...
		<?php }?>
          ...
          <textarea class="form-control" name="description" placeholder="Nhập mô tả..." rows="5" id="comment"><?php echo $row['description'];?></textarea>
          <?php if (isset($loides)) {?>
	...
	<div class="mb-3">
		...
		<input type="number" value="<?php echo $row['salary'];?>" class="form-control" id="salary" name="salary" placeholder="">
	...
	<div class="mb-3">
		...
		<div class="form-check form-check-inline">
			<input  class="form-check-input" checked type="radio" name="gioitinh" id="nam" value="1"
	...
	<div class="mb-3">
		...
		<input type="date" value="<?php echo  date("Y-m-d",strtotime($row['birthday']));?>" class="form-control" id="ngaysinh" name="ngaysinh" placeholder="Ngày sinh của bạn">
	...
	<button type="submit" name="btndangky"  class="btn btn-primary">Save</button>
          <a  class="btn btn-secondary" href="viewlist.php">Cancel</a>
</form>
